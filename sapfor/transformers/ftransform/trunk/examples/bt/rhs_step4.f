...
!STEP1: WE CAN MERGE TWO NEXT NEXTED LOOPS
c---------------------------------------------------------------------
...
!STEP2 MERGE LOOPS WITH J INDEX, THEY HAVE THE SAME BOUNDS
!STEP3 MERGE LOOPS WITH I INDEX, IF OPERATORS APPEAR AT THIS STEP

!STEP4 MERGE NEXT 4 NEXTED LOOPS, SOME LOOPS NEED TO SWAPS
...
c---------------------------------------------------------------------
!STEP1: CONVERT FIXED INDEX TO LOOP
!         do j = 1, grid_points(2)-2
!            i = 1		 
!           DO i = 1, 1 
			IF (i .eq. 1) then
...
!           ENDDO
!         enddo
           ELSE IF (i .ge. 3 .AND. i .le. grid_points(1)-4) then
!         do j = 1, grid_points(2)-2
...
!            enddo
!         enddo
            ELSE IF ( i .eq. grid_points(1)-3) then        
!         do j = 1, grid_points(2)-2
!            i = grid_points(1)-3
!           DO i = grid_points(1)-3, grid_points(1)-3
            do m = 1, 5
               rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
     >                    ( u(m,i-2,j,k) - 4.0d0*u(m,i-1,j,k) + 
     >                    6.0d0*u(m,i,j,k) - 4.0d0*u(m,i+1,j,k) )
            enddo
!          ENDDO
            ELSE IF (i .eq. grid_points(1)-2) then		  
!            i = grid_points(1)-2
!           DO i = grid_points(1)-2, grid_points(1)-2			
            do m = 1, 5
               rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
...
c---------------------------------------------------------------------
!STEP2 MERGE LOOPS WITH I INDEX, THEY HAVE THE SAME BOUNDS. THE LOOPS NEED TO SWAP BEFORE!
!STEP3 MERGE LOOPS WITH J INDEX, IF OPERATORS APPEAR AT THIS STEP
      do k = 1, grid_points(3)-2
!         do j = 1, grid_points(2)-2
!            do i = 1, grid_points(1)-2
        do i = 1, grid_points(1)-2
...
!         enddo
!        ENDDO
		  ELSE IF( j .ge. 3 .AND. j .le. grid_points(2)-4) then
...
!         j = grid_points(2)-3
!        DO j = grid_points(2)-3, grid_points(2)-3
!         do i = 1, grid_points(1)-2
            do m = 1, 5
               rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
     >                    ( u(m,i,j-2,k) - 4.0d0*u(m,i,j-1,k) + 
...
!         j = grid_points(2)-2
!        DO j = grid_points(2)-2, grid_points(2)-2		 
!         do i = 1, grid_points(1)-2
            do m = 1, 5
               rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
...
c---------------------------------------------------------------------
!STEP2 MERGE LOOPS WITH K INDEX, THEY HAVE THE SAME BOUNDS. THE LOOPS NEED TO SWAP BEFORE!
!STEP3 MERGE LOOPS WITH K INDEX, IF OPERATORS APPEAR AT THIS STEP
!      do k = 1, grid_points(3)-2
!         do j = 1, grid_points(2)-2
!            do i = 1, grid_points(1)-2
         do j = 1, grid_points(2)-2
...
!      enddo
!     ENDDO
	     ELSE IF (k .ge. 3 .AND. k .le. grid_points(3)-4) then
...
!      k = grid_points(3)-3
!     DO k = grid_points(3)-3, grid_points(3)-3
!      do j = 1, grid_points(2)-2
!         do i = 1, grid_points(1)-2
            do m = 1, 5
               rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
...
!      k = grid_points(3)-2
!     DO k = grid_points(3)-2, grid_points(3)-2
!      do j = 1, grid_points(2)-2
!         do i = 1, grid_points(1)-2
            do m = 1, 5
               rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
