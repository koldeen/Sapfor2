...
c---------------------------------------------------------------------
c         do j = 1, grid_points(2)-2
c            do i = 1, grid_points(1)-2
               do m = 1, 5
                  IF (i == 1) THEN
     ...
     >                    4.0d0*u(m,i+1,j,k) + u(m,i+2,j,k))
                  ELSE IF (i >= 3 .AND. i <= grid_points(1)-4) THEN
                     rhs(m,i,j,k) = rhs(m,i,j,k) - dssp * 
     >                    (  u(m,i-2,j,k) - 4.0d0*u(m,i-1,j,k) + 
     >                    6.0*u(m,i,j,k) - 4.0d0*u(m,i+1,j,k) + 
     >                    u(m,i+2,j,k) )
                  ELSE IF (i == grid_points(1)-3) THEN
                      rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
     >                    ( u(m,i-2,j,k) - 4.0d0*u(m,i-1,j,k) + 
     >                    6.0d0*u(m,i,j,k) - 4.0d0*u(m,i+1,j,k) )
                  ELSE IF (i == grid_points(1)-2) THEN
                      rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
...
c         enddo
c      enddo
c      if (timeron) call timer_stop(t_rhsx)

c      if (timeron) call timer_start(t_rhsy)
c---------------------------------------------------------------------
c     compute eta-direction fluxes 
c---------------------------------------------------------------------
c      do k = 1, grid_points(3)-2
c         do j = 1, grid_points(2)-2
c            do i = 1, grid_points(1)-2
               vijk = vs(i,j,k)
               vp1  = vs(i,j+1,k)
...
c            do i = 1, grid_points(1)-2
     ...
     >                    4.0d0*u(m,i,j+1,k) + u(m,i,j+2,k))
                  ELSE IF (j >= 3 .AND. j <= grid_points(2)-4) THEN
                     rhs(m,i,j,k) = rhs(m,i,j,k) - dssp * 
     >                    (  u(m,i,j-2,k) - 4.0d0*u(m,i,j-1,k) + 
     >                    6.0*u(m,i,j,k) - 4.0d0*u(m,i,j+1,k) + 
     >                    u(m,i,j+2,k) )
                  ELSE IF (j == grid_points(2)-3) THEN
                     rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
     >                    ( u(m,i,j-2,k) - 4.0d0*u(m,i,j-1,k) + 
     >                    6.0d0*u(m,i,j,k) - 4.0d0*u(m,i,j+1,k) )
                  ELSE IF (j == grid_points(2)-2) THEN
                     rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
...
c         enddo
c      enddo
c      if (timeron) call timer_stop(t_rhsy)

c      if (timeron) call timer_start(t_rhsz)
c---------------------------------------------------------------------
...
c            do i = 1, grid_points(1)-2
     ...
     >                    4.0d0*u(m,i,j,k+1) + u(m,i,j,k+2))
                  ELSE IF (k >= 3 .AND. k <= grid_points(3)-4) THEN
                     rhs(m,i,j,k) = rhs(m,i,j,k) - dssp * 
     >                    (  u(m,i,j,k-2) - 4.0d0*u(m,i,j,k-1) + 
     >                    6.0*u(m,i,j,k) - 4.0d0*u(m,i,j,k+1) + 
     >                    u(m,i,j,k+2) )
                  ELSE IF (k == grid_points(3)-3) THEN
                     rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
     >                    ( u(m,i,j,k-2) - 4.0d0*u(m,i,j,k-1) + 
     >                    6.0d0*u(m,i,j,k) - 4.0d0*u(m,i,j,k+1) )
                  ELSE IF (k == grid_points(3)-2) THEN
                     rhs(m,i,j,k) = rhs(m,i,j,k) - dssp *
...
c         enddo
c      enddo
c      if (timeron) call timer_stop(t_rhsz)
