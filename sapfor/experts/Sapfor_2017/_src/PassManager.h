...
void InitPassesDependencies(map<passes, vector<passes>> &passDepsIn, set<passes> &passesIgnoreStateDone)
{
    ...
    list({ PRIVATE_CALL_GRAPH_STAGE1, PRIVATE_CALL_GRAPH_STAGE2, MACRO_EXPANSION, CONVERT_ASSIGN_TO_LOOP }) <= Pass(SUBST_EXPR) <= Pass(SUBST_EXPR_AND_UNPARSE);
    ...
    list({ VERIFY_ENDDO, VERIFY_INCLUDE, PREPROC_SPF, PREPROC_ALLOCATES, GET_ALL_ARRAY_DECL, FILL_COMMON_BLOCKS }) << list({ CALL_GRAPH, MACRO_EXPANSION });

    list({ VERIFY_ENDDO, VERIFY_INCLUDE, PREPROC_ALLOCATES }) << list({ GET_ALL_ARRAY_DECL, FILL_COMMON_BLOCKS }) <= Pass(PREPROC_SPF);

    list({ CALL_GRAPH2, CODE_CHECKER_PASSES, SUBST_EXPR, GET_ALL_ARRAY_DECL, FILL_COMMON_BLOCKS }) <= Pass(LOOP_ANALYZER_DATA_DIST_S1);
