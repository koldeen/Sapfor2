...
void InitPassesDependencies(map<passes, vector<passes>> &passDepsIn, set<passes> &passesIgnoreStateDone, bool printTree = false)
{
    ...
    Pass(CREATE_INTER_TREE) <= list({ GCOV_PARSER, PREDICT_SCHEME, INSERT_INTER_TREE });

    list({ FILE_LINE_INFO, BUILD_INCLUDE_DEPENDENCIES }) <= Pass(CORRECT_VAR_DECL);

    Pass(DEF_USE_STAGE1) <= Pass(DEF_USE_STAGE2);

    list({ PRIVATE_CALL_GRAPH_STAGE1, PRIVATE_CALL_GRAPH_STAGE2, MACRO_EXPANSION, CONVERT_ASSIGN_TO_LOOP, DEF_USE_STAGE1, DEF_USE_STAGE2, FILL_PARALLEL_REG_FOR_SUBS, VERIFY_COMMON }) <= Pass(SUBST_EXPR) <= Pass(SUBST_EXPR_AND_UNPARSE);
        
    list({ PRIVATE_CALL_GRAPH_STAGE1, PRIVATE_CALL_GRAPH_STAGE2, PRIVATE_CALL_GRAPH_STAGE4 }) <= Pass(PRIVATE_ANALYSIS_SPF);

    Pass(LOOP_GRAPH) <= Pass(PRIVATE_CALL_GRAPH_STAGE3) <= Pass(PRIVATE_ANALYSIS_SPF) <= list({ LOOP_ANALYZER_DATA_DIST_S1, ONLY_ARRAY_GRAPH });
    ...
    list({ CALL_GRAPH2, CODE_CHECKER_PASSES, SUBST_EXPR, GET_ALL_ARRAY_DECL }) << list({ LOOP_ANALYZER_DATA_DIST_S1, ONLY_ARRAY_GRAPH });
    ...
    list({ CLEAR_SPF_DIRS, REVERT_SUBST_EXPR }) <= Pass(RESOLVE_PAR_REGIONS);
    ...
    list({ REVERT_SPF_DIRS, CONVERT_LOOP_TO_ASSIGN })  << list({ CONVERT_TO_ENDDO, CORRECT_CODE_STYLE, INSERT_INCLUDES, REMOVE_DVM_DIRS, REMOVE_DVM_DIRS_TO_COMMENTS, INLINE_PROCEDURES });
