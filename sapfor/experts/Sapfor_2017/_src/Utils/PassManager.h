...
void InitPassesDependencies(map<passes, vector<passes>> &passDepsIn, set<passes> &passesIgnoreStateDone, bool printTree = false)
{
    ...
    Pass(CONVERT_LOOP_TO_ASSIGN) <= list({ CONVERT_TO_ENDDO, CORRECT_CODE_STYLE, INSERT_INCLUDES, REMOVE_DVM_DIRS, REMOVE_DVM_DIRS_TO_COMMENTS, INLINE_PROCEDURES });

    Pass(CLEAR_SPF_DIRS) <= Pass(UNPARSE_FILE);
