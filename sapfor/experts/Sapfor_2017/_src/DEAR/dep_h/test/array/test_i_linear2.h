	...
	namespace Test
	{
		...
		{	
			...
			Dependence* DiffIDepConstSolution (int constSolution) //constSolution - решениe-константа
			{
				...
				{								
					...
					if (lIPart != 0 && constSolution <= lIPart) //lIPart == 0 => цикл выполнится только 1 раз
						dep->SetExistenceCond (DEP_EXIST, DEP_ALWAYS, DEP_EXACTLY);
					else
						dep->SetExistenceCond (DEP_NOT_EXIST, DEP_ALWAYS, DEP_EXACTLY);
				}							
			...
			Dependence* DiffIDepConstRBound ()
			{
				...
				if (tMax > tMin ||	//в [tMin, tMax] больше одной точки => т. к. точка пересечения прямых одна, то 
									//найдется точка, в которой u1 + v1 * t != u2 + v2 * t
					tMax == tMin && //tMin == tMax, но u1 + v1 * tMin != u2 + v2 * tMin
		...
		public:
			...
			{			
				...
				if (!constLBound || !constStep) return false; //если левая граница, или шаг не числа, то тест не применим
			...
			{	//DBG_START
				...
				//if (isRBConst)
				//{
				//	if ((rBound - lBound) * step < 0) //цикл iVar->GetLoopId () не выолнится не разу
				//	{
				...
				//Dependence* depUpper = new Dependence ();//(Dependence*)dep [0]->Clone (); //чтобы учесть, условие отсутствия зависимостей
				//Dependence* depInner = new Dependence ();//(Dependence*)dep [0]->Clone (); //полученное ранее

//if (step == 0) - ?
				//сделаем замену i = lBound + step * j,
				//чтобы привести уравнение к виду a * k + b * k' = c, где k, k' - номера итераций (см. диплом) 
				c -= a * lBound + b * lBound; //используются старые значения
				a *= step;
				b *= step;
