...
void resolveParRegions(std::vector<ParallelRegion*> &regions, const std::map<std::string, std::vector<FuncInfo*>> &allFuncInfo, 
                       const std::map<std::string, CommonBlock> &commonBlocks, const std::set<FuncInfo*> &allCommonFunctions, 
                       const std::map<DIST::Array*, 
                       const CommonBlock*> &allUsedCommonArrays, std::map<std::string, std::map<DIST::Array*, std::pair<SgSymbol*, SgSymbol*>>> &createdCommonArrays, 
                       std::map<std::string, SgStatement*> &createdCommonBlocks, std::set<FuncInfo*> &insertedCommonBlocks);
