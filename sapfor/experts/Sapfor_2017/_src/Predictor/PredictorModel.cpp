...
long cur_amv, cur_ps; //�������� ��� cur_amv_ptr (������ ��� = cur_amv)
long addr;
...
static int Model_distr(DIST::Array *array, const DistrVariant *distrVar, FILE *printOut = NULL)
{
    string debug = "";
    {
        ...
        tmp_params->ID = getId(array, mapIds, ids); //��� ��������
...

    {
        ...
        tmp_params->ArrayHeader = getId(array, mapArrayAddrs, arrayAddr);//��� ��������
        tmp_params->ArrayHandlePtr = getId(array, mapArrayAddrsPtr, arrayAddrPtr);//��� ��������
...
static int Model_align(DIST::Array *array, const int regId, FILE *printOut = NULL)
{
...

    {
        ...
        tmp_params->ArrayHeader = getId(array, mapArrayAddrs, arrayAddr);//��� ��������
        tmp_params->ArrayHandlePtr = getId(array, mapArrayAddrsPtr, arrayAddrPtr);//��� ��������
        tmp_params->PatternRef = getId(templ, mapArrayAddrsPtr, arrayAddrPtr); //��� ��������
        tmp_params->PatternRefPtr = getId(templ, mapArrayAddrs, arrayAddr); //��� ��������
...
int Model_rem(remote *rem)
{
    ...
    //	 ���� remote ��������� � �����	
    {
        ...
        tmp_params->BufferHeader = 0xf00000 + rem->v->addr;//��� ��������
...
static int Model_par(LoopGraph *loop, ParallelDirective *directive, FILE *printOut = NULL)
{
    ...
    if (directive->shadowRenew.size()) //SHADOW RENEW
    {
        ...
        /*
        {
            ...
            tmp_params->ShadowGroupRef = 0xb00000 + addr;//��� ��������
        ...
        for (int j = 0; j < s_few.size(); ++j)
        {
            ...
            tmp_params->ArrayHandlePtr = 0x200000 + s_few[j].v->addr; //���� 0xd00000
    ...
    if (directive->across.size()) // ACROSS
    {
        ...
        for (int j = 0; j < a_few.size(); ++j)
        {
            ...
            tmp_params->ArrayHandlePtr = 0x200000 + a_few[j].v->addr;//���� 0xd00000
    ...
    {        
        ...
        tmp_params->LoopRef = LR; //��� ��� � � crtpl
        tmp_params->PatternRef = getId(directive->arrayRef, mapArrayAddrsPtr, arrayAddrPtr); //��� �������� 
        tmp_params->PatternRefPtr = getId(directive->arrayRef, mapArrayAddrs, arrayAddr); //��� ��������
    ...
    if (directive->reduction.size() | directive->reductionLoc.size()) //REDUCTION
    {
        ...
        tmp_params->RG_ID = RID;//��� ��������
        tmp_params->RV_ID = RIDG;//��� ��������
...

    {
        ...
        //TODO: loop time!
        f.call_time = 100;		        // call time, c->time - ��� ����� �����!
        f.ret_time = 0.00000100;		// return time
        setVectorCallRet(&f);
...
int Model_par(var *v1, ali_pat *d, circle *c, reduct *r, shadow *s, across *a)
{
    ...
    if (r->is) //REDUCTION
    {
        ...
        tmp_params->RG_ID = 0xed0000 + RID;//��� ��������
        tmp_params->RV_ID = 0xe10000 + RID;//��� ��������
...
//============== Model switch
int Model(int scheme_id, long line_beg, long line_end)  //���������� ����� ��������� �������
{
    int i, j, k, q;
    ...
    while (1)
    {
        ...
        if (result[cur_res].line > line_end) break; //����� �� while

        i = result[cur_res].line; //��� ���������
        ...
        if (0 && i != line_beg && seq_index < seq_loops.size()) //seq_loops[seq_index].times!=0 
        {
            ...
            else
            {
                ...
                if (seq_loops[seq_index].times >= 1)
                {
                    ...
                    tmp_mid->copy_poss(result[cur_res].inter_mid, -1, 1); //�������� �������
                    tmp_min->copy_poss(result[cur_res].inter_min, -1, 1); //�������� �������
                    tmp_max->copy_poss(result[cur_res].inter_max, -1, 1); //�������� �������
...
//						all_scheme[curr_routine][scheme_id].Interval_tree->print(0);
                        ...
                        result[cur_res].inter_mid->copy_poss(tmp_mid, 1, (seq_loops[seq_index].times - 2)); //�������� ������� ��������� ������
                        result[cur_res].inter_max->copy_poss(tmp_max, 1, (seq_loops[seq_index].times - 2)); //�������� ������� ��������� ������
                        result[cur_res].inter_min->copy_poss(tmp_min, 1, (seq_loops[seq_index].times - 2)); //�������� ������� ��������� ������
...
//						printf("************** %.0f\n",seq_loops[seq_index].times-1);
                ...
                //				result[cur_res].inter_min->copy(tmp_res); //�� ����������
                //				result[cur_res].inter_max->copy(tmp_res); //�� ����������

            }
            //			i=is_loop[i].end_line; //������ ��� ���������� �� ������ ������ ������ �� � ���� ����.
        }
        else
        {
            int dir_type = 0;
...
//						}
            ...
            case 3:
            {
                ...
                //						if(c.time<0) 
                //							getchar();
                c.time = c.time*dir_bin[i].time / 1000000; //����� ����� ����� // ���� ����� ������ ��� ������������� �����
                c.time = c.time * (1000.0 / mach_set[curr_mach].proc_perf);  //���� ������������������ ��� ������������� �����
                ...
                //							j=0;
                //							exit(0);
                Model_par(&vars[d.varid], &d, &c, &r, &s, &a); //� ��������� ������ �����, ��������� � ��������
            ...
            case 4: // IF //���� ������ � GOTO � ��� ELSE
                ...
                poss = 0.5;
                j = 4; // ���� GOTO
                if (j < i) if (mode) printf("error: jump back - cannot make prediction - so ignore goto");//������ �����
...
//						RemoteInterval->Integrate();
                    ...
                    //was						res->Integrate();	
                                            ...
                                            // ��� ���������������� ������ ���������� ���������� ������ ����� ��������. ��������� �� ����� ������ ����� ��� ����������� ���������� �� ������
                    res->copy_poss(SeqLoopInterval, 1, dir_bin[save_i].time * (1000.0 / mach_set[curr_mach].proc_perf)); //���� ������������������
