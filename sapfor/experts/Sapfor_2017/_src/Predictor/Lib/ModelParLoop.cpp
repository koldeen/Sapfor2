...
void FuncCall::endpl()
{
	...
	/* не нужно, потому что если исползуетс€ across, то используетс€ синхронное обновлени€ гран€ми 
	/* (как на в входе (на первой итерации (мне кажетс€ там нельз€ задать асинхронный режим))
	/* так и при вычиcлении цикла across), поэтому на выходе из цикла не требуетс€ считать врем€ Shadow.
...
//	printf("DOPL %f\n",call_time);
	...
	{ 
		...
		for(k=0;k<rank;k++)
			p[k]=pp[k];
		//по другим измерени€ решетка процов имеет ширину 1
...
//		printf("rank=%d rank_mas=%d\n",rank,rank_mas);
		...
		for(i=0;i<MPSProcCount();i++)
		{
				...
				{ 
						for(j=k+1,d=1;j<rank;j++)
							d*=p[j];
					//надо prev == -1 если нет пред. процессора дл€ него по этому измерению, кот. надо ждать
						if(invers[k]) 
	...
	{ 
		double max_time;
		type_size=ParLoopInfo.ParLoop_Obj->AcrossFlag;
		//≈сли обратный отсчет в цикле то Step должен быть < 0
		for(i=0;i<MPSProcCount();i++)  
...
//grig
		...
		// провер€ем пересечение  блока minipl с локальными блоками процессоров,
		// кооректируем врем€ выполнеени€ - дл€ каждого процесссора
			for(i=0;i<MPSProcCount();i++) 
			{
    ...
    //delete minipl;
		    ...
		    if(tmp_params->Dim.size()!=0)
			{
				...
				// постоение блока выполн€ющихс€ на данный момент витков
