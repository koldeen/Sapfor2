...
static CFG_ControlFlowItem* getFuncCallsForExpr(SgExpression* e, CFG_Call* calls, CFG_ControlFlowItem *&last, CFG_CommonData* commons, SgStatement* os)
{
    ...
    if (f) 
    {
        CFG_ControlFlowItem* head = new CFG_ControlFlowItem(NULL, NULL, G_currentProcedure, calls->GetLinkToCall(e, NULL, commons));
    ...
    if (f) 
    {
        CFG_ControlFlowItem* head = new CFG_ControlFlowItem(NULL, NULL, G_currentProcedure, calls->GetLinkToCall(e->lhs(), NULL, commons));
        head->SetOriginalStatement(os);
        head->SetFunctionCall(f);
...
static CFG_ControlFlowItem* ifItem(SgStatement* stmt, CFG_ControlFlowItem* empty, SgStatement *&lastAnStmt, CFG_DoLoops* loops, bool ins, CFG_Call* calls, CFG_CommonData* commons)
{
    if (stmt == NULL)
        return empty;
    SgIfStmt* cond;
    ...
    if (stmt->variant() == ELSEIF_NODE || (!ins && (cond = isSgIfStmt(stmt)) != NULL))
    {
        ...
        CFG_ControlFlowItem* gotoEmpty = new CFG_ControlFlowItem(NULL, empty, j, NULL, G_currentProcedure);
        if (last != NULL)
            last->AddNextItem(gotoEmpty);
        else
            n = gotoEmpty;
        CFG_ControlFlowItem* tn = new CFG_ControlFlowItem(c, j, n, stmt->label(), G_currentProcedure);
...
static CFG_ControlFlowItem* switchItem(SgStatement* stmt, CFG_ControlFlowItem* empty, SgStatement *&lastAnStmt, CFG_DoLoops* loops, CFG_Call* calls, CFG_CommonData* commons)
{
    ...
    while (stmt->variant() == CASE_NODE || stmt->variant() == DEFAULT_NODE)
    {
        ...
        if ((n = getControlFlowList(stmt->lexNext(), NULL, last, lastAnStmt, loops, calls, commons)) == NULL)
            return NULL;
        j = new CFG_ControlFlowItem(G_currentProcedure);
        CFG_ControlFlowItem* gotoEmpty = new CFG_ControlFlowItem(NULL, empty, j, NULL, G_currentProcedure);
        if (last != NULL)
            last->AddNextItem(gotoEmpty);
        else
            n = gotoEmpty;
        CFG_ControlFlowItem* cond = new CFG_ControlFlowItem(c, j, n, stmt->label(), G_currentProcedure);
...
static CFG_ControlFlowItem* 
    processOneStatement(SgStatement *&stmt, CFG_ControlFlowItem *&pred, CFG_ControlFlowItem *&list, 
                        CFG_ControlFlowItem* oldcur, CFG_DoLoops* loops, CFG_Call* calls, CFG_CommonData* commons)
{
    ...
    case IF_NODE:
    {
        CFG_ControlFlowItem* emptyAfterIf = new CFG_ControlFlowItem(G_currentProcedure); //empty item to avoid second pass        
    ...
    {
        CFG_ControlFlowItem* cur = new CFG_ControlFlowItem(stmt, NULL, G_currentProcedure, stmt->variant() == PROC_STAT ? calls->GetLinkToCall(NULL, stmt, commons) : NULL);
        if (pred != NULL)
            pred->AddNextItem(cur);
        else
            list = cur;
    ...
    case LOGIF_NODE:
    {
        ...
        SgExpression* c = &(SgNotOp((cond->conditional()->copy())));
        CFG_ControlFlowItem* cur = new CFG_ControlFlowItem(c, emptyAfterIf, NULL, stmt->label(), G_currentProcedure);
    ...
    case WHILE_NODE:
    {
        ...
        if (cond->conditional())
            ...
            c = new SgValueExp(1);
        CFG_ControlFlowItem* emptyAfterWhile = new CFG_ControlFlowItem(G_currentProcedure);
        CFG_ControlFlowItem* emptyBeforeBody = new CFG_ControlFlowItem(G_currentProcedure);
        CFG_ControlFlowItem* cur = new CFG_ControlFlowItem(c, emptyAfterWhile, emptyBeforeBody, stmt->label(), G_currentProcedure);
        cur->SetOriginalStatement(cond);
        CFG_ControlFlowItem* gotoStart = new CFG_ControlFlowItem(NULL, cur, emptyAfterWhile, NULL, G_currentProcedure);
        CFG_ControlFlowItem* emptyBefore = new CFG_ControlFlowItem(NULL, (CFG_ControlFlowItem*)NULL, cur, cond->label(), G_currentProcedure);
    ...
    case FOR_NODE:
    {
        ...
        CFG_ControlFlowItem* last;
        CFG_ControlFlowItem* emptyAfterDo = new CFG_ControlFlowItem(G_currentProcedure);
        CFG_ControlFlowItem* emptyBeforeDo = new CFG_ControlFlowItem(G_currentProcedure);
        CFG_ControlFlowItem* gotoEndInitial = NULL;
        if (needs_goto) {
            SgExpression* sendc = new SgExpression(GT_OP, new SgVarRefExp(fst->symbol()), fst->end(), NULL);
            gotoEndInitial = new CFG_ControlFlowItem(sendc, emptyAfterDo, emptyBeforeDo, NULL, G_currentProcedure, true);
            gotoEndInitial->SetOriginalStatement(fst);
        }
        CFG_ControlFlowItem* stcf = new CFG_ControlFlowItem(fa, needs_goto ? gotoEndInitial : emptyBeforeDo, G_currentProcedure);
        ...
        CFG_ControlFlowItem* gotoStart = new CFG_ControlFlowItem(NULL, emptyBeforeDo, emptyAfterDo, NULL, G_currentProcedure);
        CFG_ControlFlowItem* gotoEnd = new CFG_ControlFlowItem(endc, emptyAfterDo, gotoStart, NULL, G_currentProcedure);
        gotoEnd->SetOriginalStatement(fst);
        if (needs_goto) 
            gotoEnd->SetConditionFriend(gotoEndInitial);

        CFG_ControlFlowItem* loop_d = new CFG_ControlFlowItem(add, gotoEnd, G_currentProcedure);
        loop_d->SetOriginalStatement(fst);
        CFG_ControlFlowItem* loop_emp = new CFG_ControlFlowItem(NULL, loop_d, G_currentProcedure);
        SgVarRefExp* doName = (isSgVarRefExp(stmt->expr(2)));
        int lbl = -1;
        if (!isEndDo) 
        {
        ...
        G_doLoopList->AddLoop(current_file_id, stmt, fst->start(), fst->end(), fst->step(), fst->symbol());
        if (isParLoop) 
        {
#if __SPF
        ...
        {
            CFG_ControlFlowItem* body;
            //(SgStatement *firstSt, SgStatement *lastSt, CFG_ControlFlowItem *&last, SgStatement *&lastAnStmt, CFG_DoLoops* loops, CFG_Call *calls, CFG_CommonData *commons);
    ...
    case GOTO_NODE:
    {
        ...
        CFG_ControlFlowItem* gt = new CFG_ControlFlowItem(NULL, gst->branchLabel(), NULL, gst->label(), G_currentProcedure);
        if (pred != NULL)
            pred->AddNextItem(gt);
        else
            list = gt;
    ...
    case ARITHIF_NODE:
    {
        ...
        CFG_ControlFlowItem* gt3 = new CFG_ControlFlowItem(NULL, ((SgLabelRefExp*)(stmt)->expr(1)->rhs()->rhs()->lhs())->label(), NULL, NULL, G_currentProcedure);
        CFG_ControlFlowItem* gt2 = new CFG_ControlFlowItem(&SgEqOp(*(arif->conditional()), *new SgValueExp(0)), ((SgLabelRefExp*)(stmt)->expr(1)->rhs()->lhs())->label(), gt3, NULL, G_currentProcedure);
        gt2->SetOriginalStatement(arif);
        CFG_ControlFlowItem* gt1 = new CFG_ControlFlowItem(&(*arif->conditional() < *new SgValueExp(0)), ((SgLabelRefExp*)(stmt)->expr(1)->lhs())->label(), gt2, (stmt)->label(), G_currentProcedure);
    ...
    case COMGOTO_NODE:
    {
        ...
        CFG_ControlFlowItem* gt = new CFG_ControlFlowItem(&SgEqOp(*(cgt->exp()), *new SgValueExp(++i)), lbl, NULL, cgt->label(), G_currentProcedure);
        ...
        while ((label = label->rhs()))
        {
            ...
            gt = new CFG_ControlFlowItem(&SgEqOp(*(cgt->exp()), *new SgValueExp(++i)), lbl, NULL, NULL, G_currentProcedure);
            gt->SetOriginalStatement(cgt);
            old->AddNextItem(gt);
            old = gt;
        }
    ...
    case SWITCH_NODE:
    {
        CFG_ControlFlowItem* emptyAfterSwitch = new CFG_ControlFlowItem(G_currentProcedure);
    ...
    case CONT_STAT:
    {
        CFG_ControlFlowItem* cur = new CFG_ControlFlowItem(NULL, (CFG_ControlFlowItem*)NULL, NULL, stmt->label(), G_currentProcedure);
        if (pred != NULL)
            pred->AddNextItem(cur);
        else
            list = cur;
    ...
    case CYCLE_STMT:
    {
        ...
        CFG_ControlFlowItem* cur = new CFG_ControlFlowItem(NULL, loops->GetSourceForCycle(ref), NULL, stmt->label(), G_currentProcedure);
        if (pred != NULL)
            pred->AddNextItem(cur);
        else
            list = cur;
    ...
    {
        SgSymbol* ref = stmt->symbol();
        CFG_ControlFlowItem* cur = new CFG_ControlFlowItem(NULL, loops->GetSourceForExit(ref), NULL, stmt->label(), G_currentProcedure);
        if (pred != NULL)
            pred->AddNextItem(cur);
        else
            list = cur;
    ...
    case COMM_STAT:
    {
        commons->RegisterCommonBlock(stmt, G_currentProcedure);
        return pred;
    }
    default:
        return pred;
...
{
    CFG_ControlFlowItem *list = new CFG_ControlFlowItem(G_currentProcedure);
    ...
    if (cur == NULL)
        cur = list = new CFG_ControlFlowItem(G_currentProcedure);
