	...
	register char *b;
{
	...
	void err(), err_line(),store_comment();
	...
	/* Get first character on a line */
		...
		for (p = prefix + 1; p < prefixend; p++)
			if ((c = getc(infile)) == '\n')
				...
				*p = c;
                if ((OMP_program==1) && (prefix[1]=='$') && (prefix[2]==' ')) { /*OMP*/
		...
		for (; p < prefixend; p++)
			*p = ' ';
                ...
                if((OMP_program==1) && eqn(4,prefix+1,"$omp")) {/*OMP*/
...
top:
	...
	/* Get first nonblank character on a line */  
		...
		for (; p < prefixend; p++)
			*p = ' ';
                if ((OMP_program == 1) && (prefix[1]=='$') && (prefix[2]!='O')) { /*OMP*/
			for (p = prefix; p < prefixend; p++) {/*OMP*/
				*p = ' ';/*OMP*/
				(void)ungetc(' ', infile);/*OMP*/
			}/*OMP*/
                ...
                if((HPF_program && eqn(4,prefix+1,"hpf$") && (prefix[0] != '$')) ||
                  (!HPF_program && !ftn_std && eqn(4,prefix+1,"dvm$") && (prefix[0] != '$'))) {
                   ...
                   /*-- lines_returned;*/ 
                    ...
                    /*-- lines_returned;*/  /*SPF*/
                    goto body; /*SPF*/                
                } else if ((OMP_program == 1) && eqn(4,prefix+1,"$omp")) {/*OMP*/
                    is_directive_hpf = 1; /*OMP*/
