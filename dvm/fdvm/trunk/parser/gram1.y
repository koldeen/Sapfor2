...
%type <bf_node> io read write print iofmove iofctl fmkwd ctlkwd 
...
%type <ll_node> task task_array opt_new_spec async 
	...
	| ENTRY name arglist opt_result_clause
	    {PTR_BFND p, bif;
	     ...
	     if (bif->variant == FUNC_HEDR) {
	        q = make_function($2, bif->entry.Template.symbol->type, YES);
	...
	| name
	    {  PTR_HASH oldhash;
	       ...
	       if (oldhash == HSNULL) {
	         ...
	         oldsym = oldhash->id_attr;
	         newsym = make_local_entity($1, oldsym->variant, oldsym->type,
LOCAL);
	         /* copies data in entry.Template structure and attr */
	         copy_sym_data(oldsym, newsym);	         
	           /*newsym->entry.Template.base_name = oldsym;*//*19.03.03*/
	    ...
	    { int num_triplets;
	      ...
	      if (($1->variant == VAR_REF) && 
	          (((s->variant == VARIABLE_NAME) && (s->type) &&
  	            ...
  	            (s->variant == ROUTINE_NAME))) {
	        s = $1->entry.Template.symbol =  make_function(s->parent, TYNULL, NO);
	        $1->variant = FUNC_CALL;
              }
...
ctlkwd:	  INQUIRE
           {$$ = get_bfnd(fi, INQUIRE_STAT, SMNULL, LLNULL, LLNULL, LLNULL);}
	| OPEN
...
in_out_across:  LEFTPAR opt_key_word opt_in_out dependent_array_list RIGHTPAR
                {  if($3)
...
dvm_on: ON_DIR end_spec array_ele_substring_func_ref opt_new_spec
    {              
